...
#include "i2c.h"
#include "main.h"
#define MPU6050_I2C_TIMEOUT 100  // I2C超时时间：100ms（避免硬件故障时长时间卡死）
#define i2c_write(dev_addr, reg_addr, data_size, p_data) \
HAL_I2C_Mem_Write(&hi2c1, dev_addr, reg_addr, I2C_MEMADD_SIZE_8BIT, p_data, data_size, MPU6050_I2C_TIMEOUT)
#define i2c_read(dev_addr, reg_addr, data_size, p_data) \
HAL_I2C_Mem_Read(&hi2c1, dev_addr, reg_addr, I2C_MEMADD_SIZE_8BIT, p_data, data_size, MPU6050_I2C_TIMEOUT)
...
int mpu_init(struct int_param_s *int_param)
{
    ...
    {
        delay_ms(50); // 失败后延迟并重试一次
        if (i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, data))
            return -1;
    }
    delay_ms(150); // 复位后等待更长时间（增加到150ms）

    /* Wake up chip. */
    data[0] = 0x00;
    if (i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, data))
    {
        delay_ms(50); // 失败后延迟并重试
        if (i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, data))
            return -1;
    }
    delay_ms(100); // 等待芯片完全唤醒并稳定（增加到100ms）
...
uint8_t mpu_dmp_init(void)
{
	...
	{
		delay_ms(50); // 等待MPU6050稳定
		res=mpu_set_sensors(INV_XYZ_GYRO|INV_XYZ_ACCEL);//设置所需要的传感器
		if(res)return 1;
		delay_ms(50); // 添加延迟（增加到50ms）
		
		// 降低加速度量程提高灵敏度（默认±2g，改为±2g保持，或可尝试±4g）
		res=mpu_set_accel_fsr(2);  // 2g量程（最灵敏）
		if(res)return 1;
		delay_ms(50); // 增加延迟到50ms
		...
		if(res)return 3;
		delay_ms(100); // 加载固件前延迟（增加到100ms）
		res=dmp_load_motion_driver_firmware();		//加载dmp固件
		if(res)
		{
			delay_ms(100); // 失败后重试一次
	...
	res=dmp_enable_feature(DMP_FEATURE_6X_LP_QUAT|DMP_FEATURE_TAP|	//设置dmp功能
	    ...
	    DMP_FEATURE_GYRO_CAL|DMP_FEATURE_PEDOMETER);  // 添加计步器功能
	if(res)return 6;
	res=dmp_set_fifo_rate(DEFAULT_MPU_HZ);	//设置DMP输出速率(最大不超过200Hz)
	if(res)return 7;   
	
	// 自检步骤（可选）- 如果环境干扰大可以注释掉
